...
def limpar_rec(df):
    if df is not None:
        ...
        df = df.drop_duplicates(subset=['RA_ALUNO'])
...
def gerar_relatorio(df, turma):
      
    dataatual = date.today().strftime('%d/%m/%Y')
    
    df = df_rec[(df_rec["TURMADISC"] == turma)]
    ...
    p = doc.add_paragraph()
    run = p.add_run("\n\n")
    run = p.add_run(f"SIMULADO DE RECUPERAÇÃO")
...
def gerar_excel(df_rec, turma):
    df_filtrado = df_rec[(df_rec["TURMADISC"] == turma)]
    df_filtrado['RA'] = df_filtrado['RA'].astype(str)
    df_filtrado['NOTAS'] = 0 
    colunas = ['CODTURMA', 'RA', 'ALUNO', 'NOTAS']
...
turmas_filtradas = df_rec["CODTURMA"].unique().tolist()
turma = st.selectbox("Escolha a turma", turmas_filtradas)

prova = st.selectbox("Escolha se é REC_P3", ["REC_P3"])

df_rec.rename(columns={'ALUNO_x': 'ALUNO',
                        'RA': 'RA'}, inplace=True)

df_filtrado = df_rec[(df_rec["CODTURMA"] == turma)]
st.write(f"**Alunos da Turma: {turma}**")
total = df_filtrado['ALUNO'].count()
st.write(f"**Quatidade de REC solicitadas: {total}**")
df_filtrado = df_filtrado.sort_values(by ="ALUNO", ascending= True)
st.dataframe(df_filtrado[["ALUNO", "CODTURMA", "RA_ALUNO"]])


if turma:
    excel_file = gerar_excel(df_rec, turma)
    st.download_button(
        label="⬇ Gerar e Baixar Planilha Excel",
        data=excel_file,
        file_name=f"_{turma}_{prova}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
    )
        
st.title("Criar Relatorio de Assinatura")
if turma:
    relatorio = gerar_relatorio(df_rec, turma)
    st.download_button(
        label="Gerar e Baixar Relatorio de Assinaturas",
        data= relatorio,
        file_name=f"_{turma}_{prova}.docx",
